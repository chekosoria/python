    ...
    def test_endpoints(self):
        ...
        for alias in selected_endpoints:
            ...
            if url:
                ...
                self.master.update()
                try:
                    response = requests.get(url, timeout=1800, verify=False)  # Timeout de 20 minutos y no verifica el certificado
                    ...
                    self.status_text.insert(tk.END, f"Tiempo de espera agotado al generar el reporte {alias}\n")
                        ...
                        self.status_text.insert(tk.END, f"Error al generar el reporte {alias}: {str(e)}\n")
            else:
