"""
Aplicación para probar el método GET de endpoints usando CURL
"""
...
class EndpointTesterApp:
    ...
    def load_endpoints_from_file(self):
        """Función para cargar endpoints existentes"""
    ...
    def get_endpoint_from_csv(self, alias):
        """Función para obtener la URL de los endpoints existentes a partir del alias"""
        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_file = os.path.join(current_dir, "endpoints.csv")
        if os.path.exists(config_file):
            with open(config_file, 'r', encoding="utf-8") as csvfile:
    ...
    def update_endpoint_listbox(self):
        """Función para actualizar la lista de endpoints disponibles"""
    ...
    def test_endpoints(self):
        ...
        for alias in selected_endpoints:
            ...
            if url:
                ...
                self.status_text.insert(
                    tk.END, f"No se encontró la URL para el alias {alias}\n")
